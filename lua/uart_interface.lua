...
function RestBackend:configure(params)
  ...
  if params.tx_pin ~= nil then
    ok, err = self:put_text(UART_RESOURCES.tx_pin, params.tx_pin)
    if not ok then return false, err end
  end
  if params.rx_pin ~= nil then
    ok, err = self:put_text(UART_RESOURCES.rx_pin, params.rx_pin)
    if not ok then return false, err end
  end

  ok, err = self:put_text(UART_RESOURCES.baudrate, params.baud)
  if not ok then return false, err end

  ok, err = self:put_text(UART_RESOURCES.rx_buffer_size, params.rx_size)
  if not ok then return false, err end

  ok, err = self:put_text(UART_RESOURCES.mode, params.uart_mode)
  if not ok then return false, err end
...
function RestBackend:open()
  return self:put_text(UART_RESOURCES.open_state, "true")
end
...
function RestBackend:reset_rx_cursor()
  return self:put_text(UART_RESOURCES.rx_buffer_pos, 0)
end

function RestBackend:write(payload)
  return self:put_bytes(UART_RESOURCES.tx_payload, payload)
end

function RestBackend:read_chunk()
  local ok, out = self:get_payload(UART_RESOURCES.rx_chunk)
  if not ok then
    return ""
  end
  return out or ""
...
function Module.new(cfg)
  ...
  if backend == "native" then
    ...
    error("native uart backend requested but not available")
  end
...
local function current_cfg(baud)
  return {
    backend = rawget(_G, "UART_BACKEND") or "auto",
    base_url = rawget(_G, "UART_BASE_URL"),
    client = rawget(_G, "UART_CLIENT"),
    instance = rawget(_G, "UART_INSTANCE") or 0,
    baud = tonumber(baud) or tonumber(rawget(_G, "UART_BAUD")) or 9600,
    tx_pin = rawget(_G, "UART_TX_PIN"),
    rx_pin = rawget(_G, "UART_RX_PIN"),
    rx_size = rawget(_G, "UART_RX_SIZE") or 4096,
    uart_mode = rawget(_G, "UART_MODE") or 0,
    http_timeout = rawget(_G, "UART_HTTP_TIMEOUT") or 5.0,
  }
end
...
local function create_backend(cfg)
  if cfg.backend == "native" then
    ...
    if not backend then
      return nil, nil, "native uart backend requested but not available"
    end
  ...
  if not cfg.base_url or cfg.base_url == "" or not cfg.client or cfg.client == "" then
    return nil, nil, "UART_BASE_URL and UART_CLIENT are required for REST backend"
  end

  return RestBackend:new(cfg), "rest", nil
end

function _G.uart_init(baud)
...
local function ensure_backend()
  if not active_backend then
    return nil, "uart_init(baud) must be called first"
  end
  return active_backend, nil
end

function _G.uart_open()
...
function _G.uart_write(payload)
  local backend, err = ensure_backend()
  if not backend then return false, err end
...
function _G.uart_sleep(seconds)
  local backend = active_backend
...
function _G.uart_connect(baud)
  ...
  if type(_G.uart_reset_rx_cursor) ~= "function" then
    return false, "global function uart_reset_rx_cursor is not defined"
  end
  ...
  ok, err = _G.uart_reset_rx_cursor()
  if not ok then return false, err end
