...
local cam_baud = tonumber(rawget(_G, "VC0706_BAUD") or rawget(_G, "RS485_BAUD")) or 115200
local cam_action = tostring(rawget(_G, "VC0706_ACTION") or "capture")
local cam_output = tostring(rawget(_G, "VC0706_OUTPUT") or "capture.jpg")
local cam_reset = rawget(_G, "VC0706_RESET") == true
local cam_quiet = rawget(_G, "VC0706_QUIET") == true
...
local function send_command(cmd, args, label)
  ...
  if label and label ~= "" then
    camera_log("TX " .. label .. ": " .. util_bytes_to_hex(packet))
  end
...
local function read_response(timeout_seconds, max_len, expected_len)
  ...
  if #data > 0 then
    camera_log("RX (" .. tostring(#data) .. " bytes): " .. util_bytes_to_hex(data:sub(1, math.min(#data, 64))))
  end
...
local function read_frame_buffer(length, max_retries)
  ...
  while offset < total do
    ...
    chunks[#chunks + 1] = response:sub(payload_start, payload_end)
...
local function capture_image()
  ...
  local data, read_err = read_frame_buffer(frame_len, 3)
  resume_frame()
  if not data then
    return nil, read_err
  end
...
local function run_action()
  ...
  elseif cam_action == "capture" then
    ...
    local image, cap_err = capture_image()
    if not image then
      rs485_safe_close()
