...
import { objectSpecsMiddleware } from "./api/objectspecs.js";
...
export default defineConfig({
  ...
  plugins: [
    ...
    process.env.REPORT
      ...
      : null,
    // Custom plugin to handle objectspecs API endpoint
    {
      name: 'objectspecs-middleware',
      configureServer(server) {
        server.middlewares.use(async (req, res, next) => {
          if (req.url && req.url.startsWith('/api/objectspecs/')) {
            try {
              await objectSpecsMiddleware(req, res, next);
            } catch (error) {
              console.error('Error in objectspecs middleware:', error);
              next();
            }
          } else {
            next();
  ...
  server: {
    ...
    proxy: {
      "/api": {
        ...
        target: "http://192.168.100.1:8088",
        ws: true,
        changeOrigin: true,
      },
    },
  ...
  resolve: {
    alias: {
      ...
      "@leshan-demo-servers-shared": fileURLToPath(new URL('./shared/src', import.meta.url)),
    },
    extensions: [".js", ".vue"],
  },
});
