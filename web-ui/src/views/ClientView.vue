...
import { filterModelsByRegistration } from "../js/objectspecs-api.js";
...
export default {
  ...
  methods: {
    updateModels: function () {
      this.axios
        ...
        .then((response) => {
          const filtered = filterModelsByRegistration(
            response.data,
            this.registration
          );
          this.objectdefs = Object.freeze(filtered);
        })
        .catch((err) => {
          console.error("Failed to load object specs", err);
          this.objectdefs = [];
        });
    },
  },
  computed: {
    ...
    instances: function () {
      if (this.registration && this.registration.availableInstances) {
        let instances =
